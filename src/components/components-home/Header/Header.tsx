...
function Header() {
  ...
  return (
    <header id="home" className="fixed left-0 top-0 z-50 w-full border-b border-white/10 bg-ink/80 backdrop-blur-lg">
      ...
      >
        <a href="#home" className="font-title text-xl font-bold tracking-wide text-text transition hover:text-mint">
        ...
        <ul className="hidden items-center gap-8 md:flex">
          {navItems.map((item) => (
            <li key={item.href}>
              <a className="font-medium text-muted transition hover:text-text" href={item.href}>
                {item.label}
              </a>
            </li>
          ))}
